...
router.post('/browser/launch', async (req, res) => {
  try {
    const result = await browserService.launch(req.userId);
    res.json(result);
  } catch (err) {
...
router.post('/browser/close', async (req, res) => {
  try {
    const result = await browserService.close(req.userId);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/browser/status', async (req, res) => {
  const running = await browserService.isRunning(req.userId);
  let loginStatus = { loggedIn: false, reason: 'no_browser' };
  if (running) {
    loginStatus = await authService.checkLoginStatus(req.userId);
  }
...
router.post('/browser/navigate', async (req, res) => {
  try {
    const result = await authService.navigateToLinkedIn(req.userId);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
